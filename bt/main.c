...
void Send_MotorValues(int16_t motorL, int16_t motorR)
{
  ...
  GPIO_PinState currentState = HAL_GPIO_ReadPin(GPIOC, turn_headlights_Pin);
  if (lastState == GPIO_PIN_SET && currentState == GPIO_PIN_RESET)
    headlights ^= 1;
  lastState = currentState;
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : turn_headlights_Pin r_distance_Pin */
  GPIO_InitStruct.Pin = turn_headlights_Pin|r_distance_Pin;
